...
class YayGUI(QWidget):
    ...
    def _build_search_tab(self):
        ...
        self.search_results.setHeaderLabels(['Select', 'Package', 'Version', 'Description'])
    ...
    def _collect_search_output_streaming(self, source, proc):
        ...
        for raw in lines:
            ...
            if line.startswith(' ') and self._search_ctx[source]['current'] and self._search_ctx[source]['item']:
                ...
                desc = cur['description'].strip()
                it.setText(3, desc[:120] + '...' if len(desc) > 120 else desc)
    ...
    def _search_one_finished(self, source):
        ...
        if ctx and ctx['current'] and ctx['item']:
            desc = ctx['current'].get('description', '').strip()
            ctx['item'].setText(3, desc[:120] + '...' if len(desc) > 120 else desc)
    ...
    def _show_pkg_info(self, item, _col):
        ...
        html = (
            f"<h3>{data['name']}</h3>"
            f"<p><b>Repo:</b> {data['repo']} &nbsp; <b>Version:</b> {data['version']}</p>"
            f"<p>{data['description']}</p>"
        )
        self.sidebar_text.setHtml(html)
